...
import gummworld2
from gummworld2 import geometry,model
import threading
import datetime
...
class Beast():
        def __init__(self):
                ...
                for animType in my_aniType:
                    imagesAndDurations1 = [('animation/boar/boar_%s.%s.png' % (animType, str(num).rjust(3, '0')), 0.1) for num in range(12)]
...
class MovingBeast(model.Object):
        ...
        def mio_timer_pausa(self):
                ...
                print "timer per la pausa del cinghiale "+evento_scat+"partito "+str(adesso)
                self.lanciato=True
                self.inpausa=True
        ...
        def fine_pausa(self): ##questa procedura si avvia ogni n secondi e fa muovere il cinghiale e poi fermarsi
                ...
                adesso= datetime.datetime.time(datetime.datetime.now())
                evento_scat='evento '+ str(USEREVENT+int(self.id))+ '   '
                print "finita pausa del cinghiale "+evento_scat+str(adesso)
