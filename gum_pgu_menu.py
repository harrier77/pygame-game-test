...
import os
import gum_map_and_hero
from gum_map_and_hero import *
...
class miopgumenu():
        ...
        def mio_open_mappa(self,mappa='001-1.tmx'):
                self.ripristina_menu()
                oggetto_map_hero=App_gum(resolution=(self.screen_width,self.screen_height),mappa=mappa)
                gummworld2.run(oggetto_map_hero)
        ...
        def aggiungi_voci_menu(self):
                ...
                link3 = gui.Link("Episodio II: a caccia nel deserto")
                ...
                link6 = gui.Link("Anteprima mappe")
                link6.connect(gui.CLICK,self.cliccato,"anteprima")
                self.voci_menu_contenitore.add(link6,120,400)
                
                link7 = gui.Link("Settaggi")
                link7.connect(gui.CLICK,self.cliccato,"settaggi")
                self.voci_menu_contenitore.add(link7,120,450)
                                
                link8 = gui.Link("Esci")
                link8.connect(gui.CLICK,self.cliccato,"esci")
                self.voci_menu_contenitore.add(link8,120,500)
        ...
        def main(self):
                ...
                self.screen_width= video.current_w-2
                self.screen_height= video.current_h-2
